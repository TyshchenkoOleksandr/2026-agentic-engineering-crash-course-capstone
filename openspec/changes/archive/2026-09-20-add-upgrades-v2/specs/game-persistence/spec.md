...
Stage 3 bumps the save schema to version 3 (robot / factory counts, upgrade levels), adds the
built-in v2 → v3 migration, keeps all runtime state (combo, golden button, crit effect, helper
carry, main-click carry) out of the save, and makes reset clear the runtime too.
...
`e2e/add-foundation.spec.ts` or `e2e/add-shop-v1.spec.ts` (existing; "updated" where the expected
envelope changes) or `e2e/add-upgrades-v2.spec.ts` (noted as "new"). Notation `FRESH`, `S({...})`,
`S2({...})`, `V2(...)`, `V3(...)`, `RAW_V1(b, t)` is defined in `design.md`. `L0` = `FRESH.levels`.

Storage keys unchanged: `dopamine-clicker:save`, `dopamine-clicker:save:bad`,
`dopamine-clicker:theme`, `dopamine-clicker:lang`. Current save version: `3`.
