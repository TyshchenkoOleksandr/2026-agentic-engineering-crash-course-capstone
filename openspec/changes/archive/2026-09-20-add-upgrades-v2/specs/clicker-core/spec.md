...
Stage 3 grows the game state to the v3 shape (robot / factory counts, upgrade levels). Click
values become exact real numbers (combo multipliers are fractional); whole clicks are credited to
the balance through a runtime-only main-click carry, the same idea as the helper carry.

Scenario tags: `[unit]` = Vitest test in `lib/game/state.test.ts` / `lib/game/click-value.test.ts`,
`[e2e]` = Playwright test in `e2e/add-foundation.spec.ts` (existing, unchanged). Notation `FRESH`,
`S({...})` is defined in `design.md`.
