...
Stage 3 adds the copy for Crit, Combo, Golden button, Robot, Factory and the speed-ups, level
labels, the new helper labels, the crit text, the combo meter and the golden-button texts
(24 new keys, 68 in total). Existing values are unchanged.

Scenario tags: `[unit]` = Vitest test in `lib/i18n/i18n.test.ts`. Stage 3 copy in the browser is
covered by `[e2e]` scenarios in the crit, combo, golden-button, helpers and shop deltas.
